...
class CaptchaBuilder {
     ...
     * 隨機字符
     */
    protected $characters = '123467890';
    /**
     * 圖片寬度
     */
    protected $width = 150;
    ...
    public function initialize(array $config){
        ...
        isset($config['fontSize']) && $this->fontSize = $config['fontSize'];
    ...
    public function create()
    {
        ...
        for ($i = 0; $i < $this->number; $i++) {
            ...
            imagettftext($this->image,$this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.4, $color,$this->textFont, $code[$i]);
        }
    ...
    protected function drawSineLine() {
        ...
        $px1 = 0;  // 曲線橫坐標起始位置
        $px2 = mt_rand($this->width/2, $this->width * 0.8);  // 曲線橫坐標結束位置
        ...
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->image, $px + $i , $py + $i, $color);  // 這裡(while)循環畫像素點比imagettftext和imagestring用字體大小一次畫出（不用這while循環）性能要好很多
                    $i--;
                }
            }
        }
